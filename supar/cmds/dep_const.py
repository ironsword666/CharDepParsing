...
from supar import DepConstituencyParser
from supar.cmds.cmd import parse


def main():
    parser = argparse.ArgumentParser(description='Create CRF Constituency Parser.')
    parser.set_defaults(Parser=DepConstituencyParser)
    parser.add_argument('--loss_funct', type=str, default='local', help='Loss function for training.')
    parser.add_argument('--lamb', type=float, help='Lambda for loss weight.')
    parser.add_argument('--binarization', action='store_false', help='whether to binarize constituency tree')
