...
from supar.structs import CRFConstituency, LBPConstituency, MFVIConstituency, MFVIWordBasedConstituency
...
class VIConstituencyModel(CRFConstituencyModel):
    ...
    def __init__(self,
        ...
        self.span_attn = Biaffine(n_in=n_span_mlp, bias_x=True, bias_y=False)
        self.pair_attn = Triaffine(n_in=n_pair_mlp, bias_x=True, bias_y=True)
        self.label_attn = Biaffine(n_in=n_label_mlp, n_out=n_labels, bias_x=True, bias_y=True)
...
class WordBasedVIConstituencyModel(VIConstituencyModel):
    r"""
    The implementation of Constituency Parser using variational inference.
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super().__init__(**Config().update(locals()))
        self.inference = MFVIWordBasedConstituency(max_iter)
    ...
    def forward(self, words, feats=None):
        ...
        # [batch_size, seq_len, seq_len]
        s_span = self.span_attn(span_l, span_r)
        s_span = torch.cat((s_span[:, 1:], s_span[:, :1]), dim=1)
        # [batch_size, seq_len, seq_len, seq_len]
        s_pair = self.pair_attn(pair_l, pair_r, pair_b).permute(0, 2, 3, 1)
        s_pair = torch.cat((s_pair[:, 1:], s_pair[:, :1]), dim=1)
        # [batch_size, seq_len, seq_len, n_labels]
        s_label = self.label_attn(label_l, label_r).permute(0, 2, 3, 1)
        s_label = torch.cat((s_label[:, 1:], s_label[:, :1]), dim=1)

        return s_span, s_pair, s_label


class DepConstituencyModel(CRFConstituencyModel):
    r"""
    The implementation of Constituency Parser as Deep Dependency Parsing.
    ...
    def loss(self, s_span, s_label, charts, mask, lamb=0.1, loss_funct='local'):
        r"""
        Args:
            s_span (~torch.Tensor): ``[batch_size, seq_len, seq_len]``.
                Scores of all constituents.
        ...
        # Whether a span is a gold-standard constituent.
        gold_span_mask = charts.ge(0) & mask
        # TODO: span loss, refer to cc-parser or one-order-mfvi
        if loss_funct == 'local':
            # self.span_criterion = nn.BCEWithLogitsLoss()
            span_loss, span_probs = F.binary_cross_entropy_with_logits(s_span[mask], gold_span_mask[mask].float()), s_span.sigmoid()
        elif loss_funct == 'max-margin':
            # TODO span_probs = s_span.sigmoid()
            span_probs = s_span.detach().clone()
            # loss augmented inference
            s_span[gold_span_mask] -= 1
            # [batch_size, seq_len, seq_len]
            # gradient for each span score, 1 indicates the span is a predicted constituent, 0 otherwise.
            # TODO: will cky effect back-propagation ?
            preds = cky(s_span.unsqueeze(-1), mask, grad=True).squeeze(-1)
            preds = preds != 0
            span_loss = (s_span[preds].sum() - s_span[gold_span_mask].sum()) / mask[:, 0].sum()

        # span_loss, span_probs = self.crf(s_span, mask, gold_span_mask, mbr)
        label_loss = self.criterion(s_label[gold_span_mask], charts[gold_span_mask])
        if isinstance(lamb, str):
            loss = (1 - lamb) * span_loss + lamb * label_loss
        else:
            loss = span_loss + label_loss
    ...
    def decode(self, s_span, s_label, mask):
        ...
        """
        # # local decoding   
        # s_span = (s_span > 0) & mask
        # span_preds = [sorted(i.nonzero().tolist(), key=lambda x: (x[0], -x[1])) for i in s_span]
