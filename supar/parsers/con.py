...
from supar.models import (CRFConstituencyModel, DepConstituencyModel,
                          VIConstituencyModel, WordBasedVIConstituencyModel)
...
from supar.utils.transform.tree import Tree
...
class WordBasedVIConstituencyParser(VIConstituencyParser):
    r"""
    The implementation of Constituency Parser using variational inference.
    """

    NAME = 'word-based-vi-constituency'
    MODEL = WordBasedVIConstituencyModel
...
class DepConstituencyParser(CRFConstituencyParser):
    r"""
    The implementation of Constituency Parser as Dependency Parsing.
    """

    NAME = 'dep-constituency'
    MODEL = DepConstituencyModel
    ...
    def _train(self, loader):
        ...
        for i, (words, *feats, trees, charts) in enumerate(bar, 1):
            ...
            mask = (mask.unsqueeze(1) & mask.unsqueeze(2)).triu_(1)
            s_span, s_label = self.model(words, feats)
            loss, _ = self.model.loss(s_span, s_label, charts, mask, self.args.lamb, self.args.loss_funct)
            loss = loss / self.args.update_steps
            loss.backward()
            nn.utils.clip_grad_norm_(self.model.parameters(), self.args.clip)
            if i % self.args.update_steps == 0:
    ...
    def _evaluate(self, loader):
        ...
        for words, *feats, trees, charts in loader:
            ...
            mask = (mask.unsqueeze(1) & mask.unsqueeze(2)).triu_(1)
            s_span, s_label = self.model(words, feats)
            loss, s_span = self.model.loss(s_span, s_label, charts, mask, self.args.lamb, self.args.loss_funct)
            # cky decoding with marginal probabilities
            chart_preds = self.model.decode(s_span, s_label, mask)
            # since the evaluation relies on terminals,
            # the tree should be first built and then factorized
            preds = [Tree.build(tree, [(i, j, self.CHART.vocab[label]) for i, j, label in chart])
                     for tree, chart in zip(trees, chart_preds)]
            # preds = [[(i, j, self.CHART.vocab[label]) for i, j, label in chart] for chart in chart_preds]
            total_loss += loss.item()

            # # TODO: correct the conflicting spans
            # def post_process(spans, delete_labels=None, equal_labels=None):
            #     new_spans = []
            #     for i, j, label in spans:
            #         # split collapsed unary chains
            #         labels = label.split('::')
            #         for l in labels:
            #             # DEVB
            #             if delete_labels is not None and l in delete_labels:
            #                 l = None
            #             if equal_labels is not None:
            #                 l = equal_labels.get(l, l)
            #             if l is not None:
            #                 new_spans.append((i, j, l))
            #     return new_spans

            # metric([post_process(tree, self.args.delete, self.args.equal) for tree in preds],
            #        [Tree.factorize(tree, self.args.delete, self.args.equal) for tree in trees])
    ...
    def _predict(self, loader):
        ...
        for words, *feats, trees in progress_bar(loader):
            ...
            s_span, s_label = self.model(words, feats)
            # cky decoding with marginal probabilities
            if self.args.loss_funct == 'local':
                s_span = s_span.sigmoid()
            chart_preds = self.model.decode(s_span, s_label, mask)
            preds['trees'].extend([Tree.build(tree, [(i, j, self.CHART.vocab[label]) for i, j, label in chart])
                                   for tree, chart in zip(trees, chart_preds)])
            if self.args.prob:
    ...
    def build(cls, path, min_freq=2, fix_len=20, **kwargs):
        ...
        train = Dataset(transform, args.train, binarization=args.binarization)
        if args.encoder == 'lstm':
            WORD.build(train, args.min_freq, (Embedding.load(args.embed, args.unk) if args.embed else None))
            if TAG is not None:
                TAG.build(train)
