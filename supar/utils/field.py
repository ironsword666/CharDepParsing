...
from supar.utils.embedding import TailoredEmbedding
...
class Field(RawField):
    ...
    def build(self, dataset, min_freq=1, embed=None, norm=None):
        r"""
...
class NGramField(Field):
    r""""""
    def __init__(self, *args, **kwargs):
        self.ngram = kwargs.pop('ngram') if 'ngram' in kwargs else 1
        super().__init__(*args, **kwargs)
    ...
    def build(self, dataset, min_freq=1, embed=None, norm=None):
        ...
        def build_vocab(dataset):
            n_pad = self.ngram - 1
            sequences = [[char for char in self.preprocess(seq)] + [self.pad] * n_pad
                         for seq in getattr(dataset, self.name)]
            counter = Counter("".join(chars[i:i+self.ngram])
                              for chars in sequences
                              for i in range(len(chars)-n_pad))
            return Vocab(counter, min_freq, self.specials, self.unk_index)
    ...
    def tailor_embed(self, datasets, embed_path=None, min_freq=1):
        r"""
        Constructs a :class:`~supar.utils.vocab.Vocab` object for this field from the dataset.
        If the vocabulary has already existed, this function will have no effect.
        ...
        for dataset in datasets:
            vocab.update(build_vocab(dataset))

        return TailoredEmbedding.load(embed_path, vocab)

    def transform(self, sequences):
        n_pad = self.ngram - 1
        for seq in sequences:
            seq = [char for char in self.preprocess(seq)] + [self.pad] * n_pad
            seq = ["".join(seq[i:i+self.ngram]) for i in range(len(seq)-n_pad)]
            if self.use_vocab:
                seq = [self.vocab[token] if token in self.vocab else self.unk_index for token in seq]
            if self.bos:
...
class BoolChartField(ChartField):
    ...
    def transform(self, charts: Iterable[List[List]]) -> Iterable[torch.Tensor]:
        ...
        #     yield torch.tensor(chart, dtype=torch.bool).to_sparse()
        charts = [self.preprocess(chart) for chart in charts]
        if self.use_vocab:
            charts = [[[self.vocab[i] if i is not None else -1 for i in row] for row in chart] for chart in charts]
        if self.bos:
        ...
        if self.eos:
            charts = [chart + [[self.eos_index]*len(chart[0])] for chart in charts]
        charts = [torch.stack(chart).bool().to_sparse() if isinstance(chart[0], torch.Tensor) else torch.tensor(chart, dtype=torch.bool).to_sparse() for chart in charts]
        return charts

    def compose(self, batch: Iterable[torch.Tensor]) -> torch.Tensor:
        return pad([chart.to_dense() for chart in batch], self.pad_index).to(self.device, non_blocking=True)
